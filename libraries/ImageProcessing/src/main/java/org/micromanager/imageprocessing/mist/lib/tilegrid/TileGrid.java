...
package org.micromanager.imageprocessing.mist.lib.tilegrid;
 ...
 *
 * <p>The grid can be initialized using two methods:
 *
 * <pre>
 * <code>
...
public class TileGrid<T extends ImageTile<?>> {
   ...
   private final TileGridLoader gridLoader;
   ...
   public TileGrid(StitchingAppParams params, Class<?> classType) throws InvalidClassException {
      this(params.getInputParams().getStartRow(), params.getInputParams().getStartCol(),
      * params.getInputParams().getExtentWidth(), params.getInputParams()
            .getExtentHeight(), params.getInputParams().getTileGridLoader(),
   ...
   */
   ...
   public TileGrid(StitchingAppParams params, int timeSlice, Class<?> classType)
      ...
      this(params.getInputParams().getStartRow(), params.getInputParams().getStartCol(),
      * params.getInputParams().getExtentWidth(), params.getInputParams()
                  .getExtentHeight(), params.getInputParams().getTileGridLoader(timeSlice),
   ...
   */
   ...
   @Override
   public String toString() {
      return "SubGrid: startRow: " + this.startRow
   ...
   public int getFullHeight() {
      return this.gridLoader.getGridHeight();
   }
   ...
   public int getFullWidth() {
      return this.gridLoader.getGridWidth();
   }
   ...
   private void initImageTileGrid(Class<?> imageTileClass) throws InvalidClassException {
      ...
      this.tiles = (T[][]) Array.newInstance(imageTileClass,
            this.gridLoader.getGridHeight(),
            this.gridLoader.getGridWidth());
      ...
      if (constructor == null) {
         //Log.msg(LogType.MANDATORY, "Error: constructor has changed.
         // initImageTileGrid unable to find constructor using reflection.");
      ...
      for (int r = 0; r < this.gridLoader.getGridHeight(); r++) {
         for (int c = 0; c < this.gridLoader.getGridWidth(); c++) {
            ...
            try {
               this.tiles[r][c] = (T) constructor.newInstance(
                     new File(this.imageDir, fileName), r, c, this.extentWidth, this.extentHeight,
   ...
   public List<TileGrid<T>> partitionGrid(int numSlices, GridDecomposition type) {
      ...
      switch (type) {
         case BLOCK:
            ...
            numSlicesVertical = (int) Math.ceil(((double) this.getExtentWidth()
                  / (double) sliceWidth));
         ...
         case HORIZONTAL:
            ...
            break;
         case VERTICAL:
            numSlicesVertical = (numSlices < this.getExtentWidth() ? numSlices
                  : this.getExtentWidth());

            numSlicesHorizontal = 1;

            sliceWidth = (int) Math.ceil((double) this.getExtentWidth()
                  / (double) numSlicesVertical);
      ...
      for (int h = 0; h < numSlicesHorizontal; h++) {
         for (int v = 0; v < numSlicesVertical; v++) {
            ...
            TileGrid<T> nextSlice =
                  new TileGrid<T>(this, sliceStartRow, sliceStartCol, sliceExtentWidth,
                        sliceExtentHeight);
