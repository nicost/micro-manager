 ...
 * coefficients.
 *
 * <p>Also provided are utility functions for initializing a numbering grid.
 *
 * @author Tim Blattner
 * @version 1.0
 */
public class TileGridUtils {
   ...
   public static <T> void traverseMaximumSpanningTree(TileGrid<ImageTile<T>> grid) {
      ...
      visitedTilesArray[startTile.getRow() - grid.getStartRow()][startTile.getCol()
            - grid.getStartCol()] = true;
   ...
   public static <T> void printGrid(TileGrid<ImageTile<T>> grid, TileGrid.Direction dir,
      ...
      for (int row = 0; row < grid.getExtentHeight(); row++) {
         for (int col = 0; col < grid.getExtentWidth(); col++) {
            ...
            // Log.msgnonlNoTime(Log.LogType.MANDATORY, val
            // + (col == grid.getExtentWidth() - 1 ? "" : ","));
