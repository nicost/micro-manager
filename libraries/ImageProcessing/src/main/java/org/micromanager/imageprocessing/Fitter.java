...
/**
 * Utility class that fits data to a function.
 *
 * @author nico
 * @author Jon
 */
public class Fitter {
   ...
   private static final String NO_FIT = "No fit (take max)";
    ...
    *
    * <p>TODO: investigate whether weighting (possibly automatic weighting) can
    * improve accuracy
    *
    ...
    * @param expandFactor number of points to add between each original data point
    *
   ...
   public static XYSeries getFittedSeries(XYSeries data, FunctionType type,
                                          double[] parms, int expandFactor) {
      XYSeries result = new XYSeries(data.getItemCount() * (expandFactor + 1));
      double minRange = data.getMinX();
      double maxRange = data.getMaxX();
      double xStep = (maxRange - minRange) / (data.getItemCount() * (expandFactor + 1));
      switch (type) {
         case NoFit:
            try {
               result = data.createCopy(0, data.getItemCount() - 1);
               result.setKey(data.getItemCount() * (expandFactor + 1));
         ...
         case Gaussian:
            ...
            for (int i = 0; i < data.getItemCount() * (expandFactor + 1); i++) {
    ...
    * Throws an IllegalArgumentException if the paramaters do not match the function
    *
   ...
   public static String[] getFunctionTypes() {
      return new String[] {NO_FIT, GAUSSIAN};
   }
